...
import com.krissu.querypool.CleanStatsQuery;
import com.krissu.querypool.StatsQuery;
...
public class Monitor {
	...
	private void updateStats(double x, double u, double r, double w, double m, double t) {
		try {
			Statement stmt = con.createStatement();
			StatsQuery stats = new StatsQuery(x, u, r, w, m, t);
			stmt.executeUpdate(stats.getQueryStr());
			stmt.close();
			LOG.info("Stats: Interval:" + x + ",Write Queries:" + u + ", Read:" + r + ", Write:" + w + ", Nodes:" + m + ", Read Queries:" + t);
		} catch (Exception e) {
			LOG.error(e.toString());
		}
	}
	...
	public String readPerformance() {
		...
		updateStats(x, wriCount, avgRead, avgWrite,
				c.getLoadBalancer().getReadQueue().size(), redCount);
